function index = RDiffNet_Stableversion(Net_type1,Net_type2,Loc)
%% Input
...
%% Output
%%index is the calculated indicator.

%%Within the category 类内
net_count = 0;
for net_i = 1:size(Net_type1,3)
    for net_j = net_i+1:size(Net_type1,3)
        net_count = net_count + 1;
        whincategory(net_count) = SimiNet_Reals(Net_type1(:,:,net_i),Net_type1(:,:,net_j),Loc);
    end
end
for net_i = 1:size(Net_type2,3)
    for net_j = net_i+1:size(Net_type2,3)
        net_count = net_count + 1;
        whincategory(net_count) = SimiNet_Reals(Net_type2(:,:,net_i),Net_type2(:,:,net_j),Loc);
    end
end
...
net_count = 0;
for net_i = 1:size(Net_type1,3)
    for net_j = 1:size(Net_type2,3)
        net_count = net_count + 1;
        intercategory(net_count) = SimiNet_Reals(Net_type1(:,:,net_i),Net_type2(:,:,net_j),Loc);
    end
end
index = mean(intercategory)/mean(whincategory);
end
