function [ret] = GauLap_granger_regress(X,nlags,STATFLAG)
warning off
% -----------------------------------------------------------------------
...
parfor ii=1:nvar
    ...
    [beta(:,ii),err] = GauLap_Estimate(regressors, xdep);
    %     beta(:,ii) = Weight_based_on_LaAndLa(regressors, xdep);
...
for ii=1:nvar
    ...
    for jj=1:length(caus_inx)
        eq_inx_save{ii,caus_inx(jj)} = setdiff(1:nvar,caus_inx(jj));  % vars to include in restricted regression (jj on ii)
        channel_sel(ii,caus_inx(jj)) = ii;
    end
end
eq_inx_save = eq_inx_save';
channel_sel = channel_sel';
parfor ii = 1:nvar.^2
    eq_inx = eq_inx_save{ii};
    if isempty(eq_inx) ~= 1
        chann = channel_sel(ii);
        xvec = X(chann,:)';
        xdep = xvec(nlags+1:end);          % dependent variable
        ...
        [beta_r,err] = GauLap_Estimate(regressors, xdep);
...
RSS0 = reshape(RSS0_vec,[nvar nvar])';
S = reshape(S_vec,[nvar nvar])';
