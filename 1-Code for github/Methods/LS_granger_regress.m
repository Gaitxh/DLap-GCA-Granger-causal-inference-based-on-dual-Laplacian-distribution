function [ret] = LS_granger_regress(X,nlags,STATFLAG)
warning off
% -----------------------------------------------------------------------
...
parfor ii = 1:nvar.^2
    ...
    if isempty(eq_inx) ~= 1
        ...
        beta_r = pinv(regressors)*xdep;
        temp_r = xdep-regressors*beta_r;
        RSS0_vec(ii) = sum(temp_r.^2);
        S_vec(ii) = covariance(temp_r,temp_r,nobs-nlags); % dec 08
    else
        RSS0_vec(ii) = 0;
        S_vec(ii) = 0;
    end
